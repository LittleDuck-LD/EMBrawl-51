...
class ByteStream(ChecksumEncoder):
    ...
    def writeHexa(self, data, length=0):
        self.bitoffset = 0
        if data:
    ...
    def readStringReference(self, max=90000):
        self.bitoffset = 0
        length = (self.messagePayload[self.offset] << 24)
        length += (self.messagePayload[self.offset + 1] << 16)
        length += (self.messagePayload[self.offset + 2] << 8)
    ...
    def writeUInteger(self, integer: int, length: int = 1):
        self.messagePayload += integer.to_bytes(length, self.endian, signed=False)
    ...
    def writeVLong(self, high=0, low=0):
    ...
    def readCompressedString(self, max=900000):
        ...
        if data_length <= -1:
            ...
            Debugger.warning(f"Too long String encountered, length {data_length}, max {max}")
            return b''
