...
def BackupVariable(fields,Name,Variable):
    if Name not in fields:
        fields[Name] = Variable
    return fields
def getDefaultValue(fields,VaribleList):
    for Variable in VaribleList:
        fields = BackupVariable(fields,Variable['Name'],Variable['Value'])
    return fields
...
class LoginFailedMessage(PiranhaMessage):
    ...
    def encode(self, fields):
        DefaultValue = [{
            "Name": "SecondsUntilMaintenanceEnd",
            "Value": 0
        },{
        ...
        fields = getDefaultValue(fields,DefaultValue)
        ...
        self.writeInt(fields["SecondsUntilMaintenanceEnd"])
