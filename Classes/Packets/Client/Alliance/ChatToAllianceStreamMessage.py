...
class ChatToAllianceStreamMessage(PiranhaMessage):
    ...
    def execute(message, calling_instance, fields, cryptoInit):
        ...
        if fields['Message'][0]=="/":
            fields['Command'] = fields['Message']
            ClubCmd.Handle(calling_instance, fields, cryptoInit)
            return
        ...
        message = {'StreamType': 2 ,'Message': fields['Message'], 'PlayerID': calling_instance.player.ID, 'PlayerName':calling_instance.player.Name, 'PlayerRole':club_data["Members"][str(calling_instance.player.ID[1])]["Role"], 'StreamID': [0,message_tick], "Time" : int(time.time())}
